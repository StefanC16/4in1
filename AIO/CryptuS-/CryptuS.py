...
def generate_key():
    ...
    messagebox.showinfo("Generarea cheii",
                        f"Cheia a fost generata cu succes:\n\n{key_str}")
    ...
    messagebox.showinfo("Generarea cheii",
                        "Cheia a fost copiata in clipboard.")
...
def encrypt_file():
    ...
    if encryption_key is None:
        messagebox.showerror(
            "Eroare", "Va rugam sa generati o cheie inainte de a cripta un fisier!")
        return
...
def decrypt_file():
    try:
        ...
        key = simpledialog.askstring(
            "Introduceti Cheia", "Introduceti cheia pentru decriptare:")
        if not key:
            messagebox.showwarning(
                "Atentie", "Nu s-a introdus o cheie de decriptare.")
            return
        ...
        messagebox.showinfo(
            "Decriptare", "Fisierul a fost decriptat cu succes.")
...
# Relative path to the image file from the current directory
image_folder = 'Images'
image_filename = 'imagine.jpg'

# Full path to the image file
image_path = os.path.join(current_dir, image_folder, image_filename)

# Open the image using the full path
bg_photo = Image.open(image_path)
...
generate_key_button = tk.Button(
    window, text="Genereaza cheia", bg='#FC8EFE', fg="#FFFFFF", command=generate_key)
generate_key_button.place(relx=0.5, rely=0.3, anchor=CENTER)
encrypt_button = tk.Button(window, text="Cripteaza fisier",
                           bg="#F76262", fg="#FFFFFF", command=encrypt_file)
encrypt_button.place(relx=0.5, rely=0.5, anchor=CENTER)
decrypt_button = tk.Button(window, text="Decriptare fisier",
                           bg="#86F762", fg="#FFFFFF", command=decrypt_file)
